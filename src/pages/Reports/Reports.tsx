...
const Reports: React.FC = () => {
  ...
  Array.from(new Map((usersData || []).map((u: any) => [u.user_id, u])).values())
      ...
      message.success(
        `Loaded ${normalizedBookings.length} bookings successfully`
      );
    } catch (error) {
  ...
  const handleFilter = () => {
    ...
    console.log(
      `Applied ${filterCount} filter(s), found ${filtered.length} result(s)`
    );
    setFilteredBookings(filtered);

    if (filterCount > 0) {
      message.success(
        `Found ${filtered.length} booking(s) matching your filters`
      );
    } else {
    ...
    try {
      const exportData = filteredBookings.map((booking, index) => ({
        ...
        "Created Date": booking.created_date
          ...
          : "N/A",
        "Service Type": booking.is_regular
          ? "Regular"
          : booking.is_premium
          ? "Premium"
          : booking.is_ultimate
      ...
      const fileName = `Booking_Reports_${dayjs().format(
        "YYYY-MM-DD_HHmm"
      )}.xlsx`;
      XLSX.writeFile(wb, fileName);

      message.success(
        `Exported ${exportData.length} bookings to Excel successfully!`
      );
    } catch (error) {
  ...
  const handleExportCSV = () => {
    try {
      ...
      link.setAttribute(
        "download",
        `Booking_Reports_${dayjs().format("YYYY-MM-DD_HHmm")}.csv`
      );
      ...
      message.success(
        `Exported ${csvData.length} bookings to CSV successfully!`
      );
    } catch (error) {
      console.error("Export error:", error);
      message.error("Failed to export data");
  ...
  const columns: ColumnsType<Booking> = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        dayjs(a.preferred_date).unix() - dayjs(b.preferred_date).unix(),
    },
    {
      title: "Created Date",
  ...
  return (
    <div className="booking-wrapper">
      <Card
        title={<span className="booking-title">BOOKING REPORTS</span>}
        bordered={false}
  ...
  {user.user_name.trim()}
            ...
            <Col xs={24} sm={12} md={8}>
              ...
              >
                {[
                  ...new Set(
                    bookings.map((b) => b.status?.status || "Unknown")
                  ),
                ].map((status) => (
          ...
          <Row gutter={16} className="filter-buttons">
            <Col xs={12} sm={6} md={4}>
            ...
            locale={{
              emptyText: (
                <div className="empty-text">
                  ...
                  <p>Try adjusting your filters or reset to see all bookings</p>
                </div>
              ),
            }}
            bordered
