...
const Employees: React.FC = () => {
  ...
  const fetchEmployeesData = async () => {
    ...
    try {
        ...
        Array.from(
          new Map(
            departments.map((d: any) => [
              d.departmentId,
              { label: d.departmentName, value: d.departmentId },
            ])
          ).values()
        )
      );
      ...
      setRolesData(
        roles.map((r: any) => ({ label: r.roleName, value: r.roleId }))
      );
  ...
  useEffect(() => {
    const mappedEmployees = employees.map((emp) => {
      const loc = locationsData.find(
        (l: any) => Number(l.value) === Number(emp.location_id)
      );
    ...
    if (locationFilter === "All Locations") {
      ...
      setFilteredEmployees(
        mappedEmployees.filter(
          (emp) => Number(emp.location_id) === Number(locationFilter)
        )
      );
    }
  ...
  const handleSubmitEmployee = async (values: any) => {
    try {
      if (editingEmployee) {
        ...
        await fetchEmployeesData(); // ✅ REFRESH DATA AFTER EDIT
        ...
        await fetchEmployeesData(); // ✅ REFRESH DATA AFTER ADD
  ...
  const handleDeleteEmployee = async (id: number) => {
    try {
      ...
      await fetchEmployeesData(); // ✅ REFRESH DATA AFTER DELETE
  ...
  const handleEditEmployee = (emp: any) => {
    ...
    form.setFieldsValue({
      ...
      services: emp.depts.map(
        (d: any) => departmentsdata.find((dep: any) => dep.label === d)?.value
      ),
      role_id: emp.role_id,
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 150,
      render: (code: any) => (
        <span style={{ fontWeight: "bold", color: "#0d9488" }}>{code}</span>
      ),
    },
